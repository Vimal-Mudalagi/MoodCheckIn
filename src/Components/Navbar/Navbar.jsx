...
const Navbar = () => {
  ...
  const handleUserSignout = () => {
    signOut(auth)
      ...
      .catch((error) => {
        console.error("Error while signing out the user.");
      });
  };
  ...
  return (
    <nav className="bg-[#f1f2ec] sticky z-50 top-0 shadow md:w-full">
      <div className="mycontainer flex justify-between items-center px-4 py-5 h-20">
        ...
        <ul
          className={`flex-col md:flex-row md:flex gap-4 font-semibold ${
            isMobileMenuOpen ? "flex" : "hidden"
          } md:flex absolute md:static top-20 left-0 w-full md:w-auto bg-[#f1f2ec] md:bg-transparent`}
        >
          <li>
            <NavLink
              to="/"
